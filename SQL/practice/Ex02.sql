...
예) 2014년 07월 10일
 */
select max(to_char(hire_date, 'yyyy"년"MM"월"DD"일')) "마지막 신입사원"
from employees;

/*
문제4.
부서별로 평균임금, 최고임금, 최저임금을 
부서아이디(department_id)와 함께 출력합니다.
...
select job_id, 
       avg(salary) 평균임금,
       round(avg(salary), 0) 평균임금반올림,
       max(salary) 최고임금,
       min(salary) 최저임금
from   employees
group by job_id  --참여한 컬럼이나 그룹함수만 올수있다.
...
예) 2001-01-13 토요일 
*/
select min(to_char(hire_date, 'YYYY"-"MM"-"DD DAY')) 입사일 --가장 빨리 입사한사람
from employees;

/*
문제7.
평균임금과 최저임금의 차이가 2000 미만인 부서(department_id), 
평균임금, 최저임금 그리고 (평균임금 – 최저임금)를 (평균임금 – 최저임금)의 내림차순으로 
정렬해서 출력하세요.
*/
select department_id,
       trunc(avg(salary),0) 평균임금,
       min(salary) 최저임금,
       (trunc(avg(salary),0)-min(salary)) "평균임금 - 최저임금"
from   employees
where department_id is not null
group by department_id
having (avg(salary)-min(salary)) < 2000   --그룹으로 지정한 다음 설정
...
select manager_id,
       ...
       to_char(hire_date, 'YYYY"년"MM"월"DD') 입사일,
       min(salary) 최소급여,
       max(salary) 최대급여
from   employees
